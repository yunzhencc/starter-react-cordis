...
const virtualModuleId = 'virtual:cordis-example-basic-boot';
const resolvedVirtualModuleId = `\0${virtualModuleId}`;

function renderWebBootVirtualModule(graph: WebBootGraph) {
...
function emitWebBootGraph(bundle: Pick<PluginContext, 'emitFile'>, graph: WebBootGraph) {
...
export function cordisWebBoot(configPath = resolve(import.meta.dirname, 'cordis.yml')): Plugin {
  ...
  return {
    name: 'cordis-example-basic-boot',
    buildStart() {
      loadGraph();
    },
    generateBundle() {
